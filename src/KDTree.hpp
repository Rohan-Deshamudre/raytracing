...
public:
	...
	KDTree(Tucano::Mesh pointerToMesh, Eigen::Matrix4f shapeMatrix, std::vector<int> _faces, Eigen::Vector3f _min, Eigen::Vector3f _max) {
		...
		if (faces.size() > NUM_OF_FACES) {
			...
			KDTree rightBox = KDTree(*mesh, shapeModelMatrix, facesOfRightBox, minRight, maxV);
			KDTree leftBox = KDTree(*mesh, shapeModelMatrix, facesOfLeftBox, minV, maxLeft);
